...
function cueTime(ms: number) {
  return formatTimestamp(ms).slice(3, 8);
}
...
export default function Home() {
  ...
  function replaceCues(next: CaptionCue[], remember = true) {
    ...
    setCues(normalized);
    setActiveCueId(normalized[0]?.id ?? "");
  }
  ...
  const installCommand = "npx -y github:hellomileshsu/freecap#v1 mcp";
  const mcpConfig = `{"mcpServers":{"freecap":{"command":"npx","args":["-y","github:hellomileshsu/freecap#v1","mcp"]}}}`;
  ...
  return (
    <main className="freecap-shell">
      <header className="topbar">
        ...
        <div className="topbar-meta"><span className="privacy-pill"><span className="status-dot" />本機優先</span><span className="version-label">v0.1 · GPL-3.0</span><button className="icon-button" type="button" onClick={() => setShowMcp(true)} aria-label="開啟 MCP 安裝說明">⌘</button></div>
      </header>
